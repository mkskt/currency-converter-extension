...
const currencyRegex = /(?:(\$|€|£|zł|¥|₹|₩|A\$|C\$|[A-Z]{3})\s*([0-9,.]+))|(([0-9,.]+)\s*(\$|€|£|zł|¥|₹|₩|A\$|C\$|[A-Z]{3}))/i;
...
document.addEventListener('mouseup', (e) => {
    ...
    chrome.storage.sync.get({targetCurrencies: ['EUR'], modifierKey: 'none', blacklist: ''}, (settings) => {
        ...
        if (settings.blacklist) {
            const domains = settings.blacklist.split('\n').map(d => d.trim().toLowerCase()).filter(Boolean);
            const isBlocked = domains.some(domain => currentHost === domain || currentHost.endsWith('.' + domain));
            if (isBlocked) return;
        }
